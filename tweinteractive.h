...
/*!
 * メニュー構成を切り替えるための関数ポインタ群
 */
typedef struct _TWEINTRCT_sFuncs {
	uint8 u8MenuId;
	uint8 *pu8MenuString;
	void (*pf_vSerUpdateScreen)(TWEINTRCT_tsContext *psIntr);
	void (*pf_vProcessInputByte)(TWEINTRCT_tsContext *psIntr, uint8 u8Byte);
...
typedef struct _TWEINTRCT_sContext {
	...
	uint8 u8screen_max;		//! メニューの最大数 (0,1,2 の３メニューある場合は 3 になる)
...
typedef enum {
	...
	E_TWEINRCT_OP_GET_KINDNAME,				//! 種別KIND名文字列を返す。 (u32Arg1: n/a u32Arg2: n/a vpArg: uint8 **/確保済みバッファ16バイトまたは文字列へのポインタ retrun: n/a)
