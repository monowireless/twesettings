...
#define TWESTG_INIT_FINAL(NAME,PSFINAL) \
	...
	TWESTG_vInit_tsFinal((PSFINAL), NULL, __TWESTG_##NAME##_sData, &__TWESTG_##NAME##_sBufStrPool, __TWESTG_##NAME##_apEle, \
            __TWESTG_##NAME##_auShortcutKeys, sizeof(__TWESTG_##NAME##_auShortcutKeys), __TWESTG_##NAME##_uData, sizeof(__TWESTG_##NAME##_uData) / 4);
...
typedef struct _TWESTG_sElement {
	...
	struct {
		TWESTG_tuDatum d1;
		TWESTG_tuDatum d2;
		TWE_APIRET(*fpOp)(struct _TWESTG_sElement* pMe, TWESTG_tuDatum* pDatum, uint16 u16OpId, TWE_tsBuffer* pBuf);
		void* pvOpt;
	} sValidate;
} TWESTG_tsElement;
...
typedef struct _TWESTG_sFinal {
    ...
    TWESTG_tsDatum *asDatum;	//! データ構造体の配列へのポインタ
